...
export function runReplay(symbol:string,cursor=240){const session=ReplaySession.fromBars(seededBars(symbol),symbol),pipe=new QuantPipeline();const end=session.events[Math.min(cursor,session.events.length)-1]?.timestamp;session.replay(e=>pipe.onBar(e),undefined,end);return{session,snapshot:pipe.snapshot()};}
export function runBacktest(symbol:string,seed=17){
 const session=ReplaySession.fromBars(seededBars(symbol,240,seed),symbol),pipe=new QuantPipeline(); session.replay(e=>pipe.onBar(e)); const s=pipe.snapshot(); const returnPct=(s.equity-100000)/1000; return {symbol,events:s.eventsProcessed,equity:s.equity,realizedPnl:s.realizedPnl,unrealizedPnl:s.unrealizedPnl,drawdown:s.drawdown,exposure:s.exposure,fills:s.fills,returnPct};
}