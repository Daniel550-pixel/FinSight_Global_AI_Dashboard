...
export class QuantPipeline{
 ...
 snapshot(){return{position:this.position,cash:this.cash,lastPrice:this.lastPrice,realizedPnl:this.realizedPnl,equity:this.cash+this.position*this.lastPrice,eventsProcessed:this.events,openOrders:this.executor.orders.size,fills:this.executor.fills.length,analysis:this.analysis,orderBook:this.book,depth:this.lastBook,imbalance:this.book.imbalance(5),spread:this.book.spread(),mid:this.book.mid(),microprice:this.book.microprice()}}
}