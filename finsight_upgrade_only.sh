#!/bin/bash
# FinSight Upgrade Only - Git Bash
# Auteur: ChatGPT GPT-5 mini
# Doel: Safe restore & upgrade, zonder Streamlit te starten
# Gebruik: bash finsight_upgrade_only.sh

set -e
trap 'echo "[ERROR] Er is iets misgegaan, terminal blijft open voor debug."' ERR

echo "[INFO] Start safe FinSight upgrade (zonder Streamlit)..."
...
# 3️⃣ START_STREAMLIT-check toevoegen (maar script blijft niet starten)
if [ -f finSight_master.py ]; then
    cat > temp_fs_master.py <<'EOPY'
import os
START_STREAMLIT = os.getenv("START_STREAMLIT","false").lower() == "true"
if not START_STREAMLIT:
    print("[INFO] Streamlit auto-start is uitgeschakeld. Terminal blijft vrij voor coderen.")
EOPY
...
# 4️⃣ Core modules herstellen/aanmaken
...
# 6️⃣ Git backup (veilig)
if git rev-parse --is-inside-work-tree > /dev/null 2>&1; then
    git add .
    git commit -m "Safe upgrade-only $(date '+%Y-%m-%d %H:%M:%S')" || echo "[INFO] Geen wijzigingen om te committen"
...
# 7️⃣ Eindstatus
echo "[INFO] Safe upgrade voltooid."
echo "[INFO] Backup: finSight_master_backup.py"
echo "[INFO] Terminal blijft vrij voor coderen."
